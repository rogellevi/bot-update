...
async def boton_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "|" in data:
        ...
        if accion == "ver":
            ...
            mensajes = {
                "agregar": "Envía el mensaje como: clave valor",
                "borrar": "Envía la clave que deseas borrar",
                "subir": "Envía el archivo JSON para reemplazar"
            }
...
async def recibir_texto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if modo == "agregar":
        try:
            ...
            await update.message.reply_text(f"✔ Guardado: {clave} = {valor}")
        except:
...
async def recibir_archivo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if archivo.mime_type != "application/json":
        return await update.message.reply_text("❌ Solo se aceptan archivos JSON.")
    file_name = context.user_data.get("archivo_seleccionado")
...
[Unit]
Description=Bot JSON Interactivo con HTTPS
