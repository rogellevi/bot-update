...
async def boton_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("menu|"):
        files = listar_json()
        if not files:
            await query.edit_message_text("📁 No hay archivos JSON disponibles.")
            return
        keyboard = [[InlineKeyboardButton(f, callback_data=f"archivo|{f}")] for f in files]
        await query.edit_message_text("📁 Archivos JSON disponibles:", reply_markup=InlineKeyboardMarkup(keyboard))
        context.user_data.clear()
        return
    ...
    if "|" in data:
        ...
        if accion == "ver":
            ...
            keyboard = [[InlineKeyboardButton("⬅ Regresar", callback_data="menu|")]]
